    ...
    <div class="flex items-center justify-between w-full py-10 text-2xl capitalize px-7">
        <div class="flex items-center justify-between w-full gap-3 text-lg font-bold capitalize">
            <div class="flex items-center gap-3">
                ...
                @if ($dataPerDivisi->icon)
                    <div class="px-3 py-3 text-2xl bg-gray-300 rounded-full">
                        <img class="object-cover w-8"
                            src="{{ asset('storage/divisi/' . basename($dataPerDivisi->icon)) }}" alt="icon divisi">
                    </div>
                @else
            ...
            <div>
                <button data-modal-target="edit{{ $dataPerDivisi->id }}"
                    data-modal-toggle="edit{{ $dataPerDivisi->id }}"
                    class="px-2.5 py-1.5 ml-4 text-2xl font-medium text-center text-white bg-yellow-400 rounded-lg hover:bg-yellow-300 hover:text-gray-800">
                    <i class="ri-edit-2-line"></i>
                </button>
                <button data-modal-target="delete{{ $dataPerDivisi->id }}"
                    data-modal-toggle="delete{{ $dataPerDivisi->id }}"
            ...
            <x-admin.popup-divisi title="edit" id="edit{{ $dataPerDivisi->id }}" method='PUT' :action="route('dashboard.divisi.Edit', $dataPerDivisi->id)"
                :data="$dataPerDivisi" />
            <x-admin.popup-divisi title="delete" id="delete{{ $dataPerDivisi->id }}" :action="route('dashboard.divisi.Delete', $dataPerDivisi->id)"
                :data="$dataPerDivisi" />
        </div>
    </div>
