@props(['title', 'action', 'id', 'data'=>null, 'method'=>null, 'tanggal'=>null])
@if ($title == 'delete')
{{-- model delete --}}
    ...
    <div class="relative w-full max-w-xl max-h-full p-4">
        <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
            <div class="p-4 text-center md:p-5">
                ...
                <h3 class="mb-5 text-lg font-semibold text-gray-700 dark:text-gray-400">Apakah Anda Yakin ingin menghapus Absensi {{ $data->nama }} <br><span class="text-base font-medium text-red-600 font-italic" >(<span class="text-yellow-500">Attention: </span>Mengakibatkan Data Absensi Karyawan <span class="text-yellow-500">{{ $data->nama }} </span> pada tanggal <span class="text-yellow-500">{{ $tanggal }}</span> terhapus)</span> </h3>
                ...
                <form action="{{ $action }}" method="POST">
                    ...
                    <div class="grid grid-cols-2 gap-4 mb-4">
                        ...
                        <div class="col-span-1">
                            ...
                            <input type="text" name="jam_izin" id="time-picker" class="bg-gray-50 border border-gray-300 text-gray-900 rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500"
                            placeholder="{{ $data->jam_izin ?? 'Jam izin...' }}" />
                        </div>
                        <div class="col-span-1">
                            <label for="jam_selesai_izin" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Jam Selesai Izin</label>
                            <input type="text" name="jam_selesai_izin" id="time-picker" class="bg-gray-50 border border-gray-300 text-gray-900 rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500"
                            placeholder="{{ $data->jam_selesai_izin ?? 'jam_selesai_izin...' }}" />
                        </div>
                        <div class="col-span-2">
                            <label for="status_kehadiran" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">
                                Status Kehadiran
                            </label>
                            <select name="status_kehadiran" class="bg-gray-50 border border-gray-300 text-gray-900 rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500">
                                <option value="Masuk" {{ $data->status_kehadiran == 'Masuk' ? 'selected' : '' }}>Masuk</option>
