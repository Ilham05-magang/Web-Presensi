...
@php
    if ($data != null) {
        $tanggal = \Carbon\Carbon::parse($data->tanggal_libur)->translatedFormat('d-F-Y');
    }
@endphp
@if ($title == 'delete')
    {{-- model delete --}}
            ...
            <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                <div class="p-4 text-center md:p-5">
                    ...
                    <h3 class="mb-5 text-lg font-semibold text-gray-700 dark:text-gray-400">Apakah Anda Yakin ingin
                        menghapus {{ $data->nama }}<br><span
                            class="text-base font-medium text-red-600 font-italic">(<span
                                class="text-yellow-500">Attention: </span>Mengakibatkan <span
                                class="text-yellow-500">{{ $data->nama }} </span> Pada karyawan terhapus)</span> </h3>
        ...
        <div class="relative w-full max-w-xl max-h-full p-4">
            <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                <div class="p-4 text-center md:p-5">
                    ...
                    <h3 class="mb-5 text-lg font-semibold text-gray-700 dark:text-gray-400">Apakah Anda Yakin ingin
                        menghapus {{ $tanggal ?? '' }}<br><span
                            class="text-base font-medium text-red-600 font-italic">(<span
                                class="text-yellow-500">Attention: </span>Mengakibatkan <span
                                class="text-yellow-500">{{ $tanggal ?? '' }} </span> Pada tanggal libur
                            terhapus)</span> </h3>
                    <form action="{{ $action }}" method="POST">
                        @csrf
                        @method('DELETE')
        ...
        <div class="relative w-full max-w-xl max-h-full p-4">
            <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                <div class="p-4 text-center md:p-5">
                    ...
                    <h3 class="mb-5 text-lg font-semibold text-gray-700 dark:text-gray-400">Apakah Anda Yakin ingin
                        menghapus {{ $data->quote ?? '' }}<br><span
                            class="text-base font-medium text-red-600 font-italic">(<span
                                class="text-yellow-500">Attention: </span>Mengakibatkan <span
                                class="text-yellow-500">"{{ $data->quote ?? '' }}" </span> Pada Quotes
                            terhapus)</span> </h3>
                    <form action="{{ $action }}" method="POST">
                        @csrf
                        @method('DELETE')
                            ...
                            @if ($title == 'Tambah Kantor' || $title == 'Edit Kantor')
                                <div class="col-span-2">
                                    ...
                                    <input type="text" name="nama" id="nama"
                                        ...
                                        placeholder="{{ $data->nama ?? 'Nama Kantor...' }}" value="" required>
                                </div>
                                <div class="col-span-2">
                                    ...
                                    <input type="text" name="link_gmaps" id="link_gmaps"
                                        class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500"
                                        placeholder="{{ $data->link_gmaps ?? 'Link Gmaps...' }}" value="">
                                </div>
                            @elseif($title == 'Tambah Tanggal' ||$title == 'Edit Tanggal')
                                <div class="relative col-span-2">
                                    <input required name="tanggal_libur" type="date"
                                        class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                                        placeholder="Select date" value="{{$data->tanggal_libur ?? ''}}">
                                </div>
                            @elseif($title == 'Edit Quotes' || $title == 'Tambah Quotes')
                                <div class="col-span-2">
                                    <label for="Quote
                                    class="block mb-2 text-sm
                                        font-medium text-gray-900 dark:text-white">Quote</label>
                                    <input type="text" name="quote" id="quote"
                                        class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500"
                                        placeholder="{{ $data->quote ?? 'Ketikan quote' }}" value="{{$data->quote ?? ''}}">
                                    <!-- Ensure value is pre-filled -->
                                </div>
                            @else
                                <div class="col-span-2">
                                    <label for="nama"
                                    ...
                                    <input type="text" name="nama" id="nama"
                                        class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500"
                                        placeholder="{{ $data->nama ?? 'Nama Shift...' }}" value="">
                                    <!-- Ensure value is pre-filled -->
                                </div>
