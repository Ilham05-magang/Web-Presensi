<x-layout.layout>
    ...
    <div class="flex relative py-5 px-16 max-md:px-8 mx-auto justify-center items-center gap-16 max-md:flex-col ">
        <div class="flex flex-col flex-none py-10 gap-3 tracking-[.13em] max-md:hidden">
            ...
            <form class="flex" action="{{ route('presensi') }}" method="POST">
                @method('PUT')
                @csrf
                <button {{$absensi->jam_pulang ? 'disabled' : '' }} type="submit"
                    class="disabled:opacity-20 bg-button w-full text-white font-semibold rounded-xl px-3 py-3 text-xs justify-center gap-1 ">{{ $titleButton ?? 'Masuk' }}</button>
            </form>
            <button data-modal-target="modal-add" data-modal-toggle="modal-add"
                class="px-3 py-3 text-xs bg-button text-white font-semibold rounded-xl justify-center gap-1 flex"><img
                    src="assets/file-add-icon.svg" class="w-4" alt="add-log"> Log Activity</button>
            <a href="/history-log"
                class="cursor-pointer bg-button text-white font-semibold rounded-xl px-3 py-3 text-xs justify-center gap-1 flex"><img
                    src="assets/file-history-icon.svg" class="w-4" alt="history-log"> History Log Activity</a>
        </div>
                    ...
                    <div class="flex h-full items-center gap-4 ">
                        <div class="flex border w-3/5 p-3 h-32 items-center justify-center border-red-600 rounded-md">
                            <div class="text-xs text-center text-red-500">
                                <li>
                                    Kemarin anda absen pulang di kost jangan diulang! </li>
                            </div>
                        </div>
                        <div
                            class="flex gap-1 h-32 p-3 items-center w-2/5 justify-center flex-col flex-auto border border-red-600 rounded-md">
                            <div class="text-xs text-center  text-red-500">Anda memiliki kekurangan jam kerja</div>
                            <div
                                class="w-full border border-red-500 rounded-sm bg-gray-200 text-xs text-red-500 font-semibold text-center ">
                                -14:00:00
                            </div>
